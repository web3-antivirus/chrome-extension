...
import { ApprovesDetails, SwapDetails } from 'modules/analyze/Scan/interfaces';
...
export const getDetailsMessage = (swap: SwapDetails | null, permissionRequest: ApprovesDetails[] | null): ReactNode => {
  const getSwapMessage = (swapData: SwapPart[]) => swapData.map(
    ({ isToken, item }) => (isToken
      ? `${String((item).amount)} ${item.symbol}`
      : `${item.count > 1 ? `${item.count} ` : ''} ${getNftName((item).id, item.name)}`),
  ).join(' and ');
